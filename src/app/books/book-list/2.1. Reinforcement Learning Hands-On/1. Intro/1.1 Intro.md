...
- Prelogue
  ...
  - This book introduced the implementations of RL algorithms using [Gym library](https://github.com/Farama-Foundation/Gymnasium), which provides a wide range of environments yet without RL algorithms
  - for RL algorithms, refer to Stable-Baselines3 (SB3), which provides _A library of RL algorithms_ (PPO, DQN, SAC, A2C, etc.) implemented in PyTorch and is built on top of Gymnasium
...
- **2. Markov reward process** (MRP)
  ...
  - _**value of the state**_: expected/average return, more practical than _return_
