...
const PaperCard = async ({
    ...
    return <div className="lg:basis-1/4 md:basis-full">
        <div className="h-fit px-3 py-6 mx-1 my-1 border border-1 rounded rounded-lg">
            ...
            {/* row 1 */}
            <h4 className="font-bold text-lg capitalize">{paper.title}</h4>

            {/* row 2 */}
            <div className="flex flex-row flex-wrap space-x-1 pt-1">
                {paper.authors.map(a =>
            ...
            <div className="flex flex-row flex-wrap pt-2">
                {paper.labels.map(l => <span className="border border-1 px-2 mx-1 mt-1 rounded-lg text-xs hover:border-yellow-100">{l}</span>)}
            </div>
        </div>
    </div>
}
